...
function ApplicationWindow() {
	...
	// create a session
	session = opentok.createOTSession({ id: '1sdemo00855f8290f8efa648d9347d718f7e06fd' });
	...
	session.connect({ apiKey:'1127', token: 'devtoken' });
 ...
 * 		@property {Session} session The session (proxy object) which just connected
 */
function sessionConnectedHandler(event) {
	var i, newSubscriber;
	
	// For all of the existing streams in the session, create a subscriber
	for (i = 0; i < event.session.streams.length; i++) {
		// Create a subscriber object (its view is not necessarily on screen, but data is being streamed)
		newSubscriber = opentok.createSubscriber({ stream: event.session.streams[i] });
 ...
 * 
 * @param {object} event Information about the event
 * 		@property {Session} session The session (proxy object) which just disconnected
 */
 ...
 * 
 * @param {object} event Information about the event
 * 		@property {Error} session The session (proxy object) which contains the new stream
